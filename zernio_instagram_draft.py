...
def choose_uploaded_video(media_history):
...
def ask_for_manual_caption():
    ...
    if not caption:
        print("Caption is empty. Draft creation canceled.")
        return None
...
def build_draft_payload(account, video_record, content):
    return {
        "content": content,
        "isDraft": True,
        "platforms": [
...
def confirm_draft(account, video_record, content):
    print()
    print("Draft preview")
    print("=============")
    ...
    answer = input("Type DRAFT to create this Zernio Instagram Reel draft: ").strip()

    if answer != "DRAFT":
        print("Draft creation canceled.")
        return False

    return True


def create_instagram_reel_draft(api_key, requests, payload):
    try:
        ...
        print_http_error(error, "draft request")
        return None
    except requests.exceptions.RequestException:
        print("Error: Could not connect to Zernio to create the draft.")
        return None

    data = get_json_response(response, "the draft response")

    if not isinstance(data, dict):
        print("Error: Zernio returned an unexpected draft response format.")
        return None

    post = data.get("post")

    if not isinstance(post, dict):
        print("Error: Zernio draft response did not include post details.")
        return None
...
def main():
    print("Zernio Instagram Reel draft creator")
    print("===================================")
    ...
    selected_video = choose_uploaded_video(media_history)
    ...
    if not confirm_draft(selected_account, selected_video, content):
        return

    payload = build_draft_payload(selected_account, selected_video, content)
    post = create_instagram_reel_draft(api_key, requests, payload)
    ...
    print("Zernio Instagram Reel draft created.")
