...
POST_HISTORY_FILE = PROJECT_FOLDER / "zernio_tiktok_post_history.json"
...
def response_mentions_public_privacy_error(response):
    if response is None:
        return False
...
def print_http_error(error, action):
    ...
    if status_code == 400:
        if response_mentions_public_privacy_error(response):
            print("Error: TikTok public publishing is not available for this account.")
            print(
                "The connected TikTok account may not permit public API publishing "
                "with PUBLIC_TO_EVERYONE."
            )
        else:
...
def fetch_tiktok_posts(api_key, requests):
    try:
        response = requests.get(
            ZERNIO_POSTS_URL,
            headers={
        ...
        print("Error: Could not connect to Zernio to load existing TikTok posts.")
        return None

    return get_json_response(response, "the published TikTok posts list")
...
def get_tiktok_accounts(accounts):
    tiktok_accounts = []
    ...
    for account in accounts:
        ...
        if not is_tiktok_account(account):
            continue
        ...
        tiktok_accounts.append(account)

    return tiktok_accounts


def choose_tiktok_account(accounts):
    tiktok_accounts = get_tiktok_accounts(accounts)

    if not tiktok_accounts:
        print("No active TikTok accounts were found in Zernio.")
        print("Connect a TikTok account in the Zernio dashboard first.")
        return None

    print("Connected TikTok accounts:")

    for number, account in enumerate(tiktok_accounts, start=1):
    ...
    choice = input("Choose one TikTok account by number: ").strip()
    ...
    if selected_number < 1 or selected_number > len(tiktok_accounts):
        print("Invalid choice. Number is not in the list.")
        return None

    return tiktok_accounts[selected_number - 1]


def choose_upload_mode():
    print("Choose TikTok upload mode:")
...
def local_history_says_published(post_history, video_path):
    for record in post_history:
        ...
        if platform != "tiktok" or not is_not_draft:
            continue
...
def platform_entry_targets_tiktok(platform_entry):
...
def post_targets_selected_account(post, selected_account):
    ...
    platform_entries = [
        ...
        if platform_entry_targets_tiktok(platform_entry)
    ]
    ...
    return post_targets_tiktok(post)
...
def zernio_posts_say_published(zernio_posts, video_path, selected_account):
    for post in zernio_posts:
        ...
        if not post_targets_tiktok(post):
            continue
...
def get_unpublished_videos(
    ...
    for video_path in exported_videos:
        ...
        if already_published_reason:
            ...
            print(
                f"Hidden already-published TikTok video: {video_path.name} "
...
def choose_video(videos):
    print("TikTok videos available to publish:")
...
def show_batch_video_list(videos):
    print("TikTok videos that will be processed:")
...
def confirm_publish_all(pause_seconds):
    ...
    answer = input(
        "Type PUBLISH ALL to upload and publish all listed TikTok videos: "
    ).strip()
...
def combine_and_limit_caption(caption, hashtags):
    ...
    print("Warning: TikTok caption was longer than 2,200 characters.")
    print("The caption was safely shortened while preserving hashtags where possible.")

    if len(hashtags) + 2 >= TIKTOK_CAPTION_LIMIT:
        return hashtags[:TIKTOK_CAPTION_LIMIT]

    available_caption_length = TIKTOK_CAPTION_LIMIT - len(hashtags) - 2
    shortened_caption = caption[:available_caption_length].rstrip()

    return f"{shortened_caption}\n\n{hashtags}"
...
def read_caption(video_path):
    ...
    return combine_and_limit_caption(caption, hashtags)
...
def confirm_publish(account, video_path, content):
    print()
    print("TikTok publish preview")
    print("======================")
    print(f"TikTok account: {get_display_name(account)}")
    print(f"Username: {get_username(account)}")
    print(f"Account ID: {get_account_id(account)}")
    print(f"Video filename: {video_path.name}")
    print("Caption:")
    print(content)
    print(f"Privacy level: {TIKTOK_PRIVACY_LEVEL}")
    print("Comments enabled: yes")
    print("Duet enabled: yes")
    print("Stitch enabled: yes")
...
def create_tiktok_publish(api_key, requests, payload):
    try:
        response = requests.post(
            ZERNIO_POSTS_URL,
            headers={
        ...
        print_http_error(error, "TikTok publish request")
        return None
    except requests.exceptions.RequestException:
        print("Error: Could not connect to Zernio to publish the TikTok video.")
        return None

    data = get_json_response(response, "the TikTok publish response")

    if not isinstance(data, dict):
        print("Error: Zernio returned an unexpected TikTok publish response format.")
        return None

    post = data.get("post")

    if not isinstance(post, dict):
        print("Error: Zernio TikTok response did not include post details.")
        return None
...
def publish_video(
    ...
    post = create_tiktok_publish(api_key, requests, payload)
    ...
    print("TikTok publish request created.")
...
def save_post_record(post_history, post, account, media_record, content):
    ...
    if post_record_exists(post_history, post_id):
        print("This TikTok post is already saved in zernio_tiktok_post_history.json.")
        return
...
def run_batch_mode(
    ...
    print(f"Already published TikTok videos skipped: {already_published_count}")
    print(f"Successfully submitted: {successful_count}")
    print(f"Failed: {failed_count}")
    print(f"Videos skipped because caption info was missing: {skipped_caption_count}")


def main():
    print("Unified TikTok uploader")
    print("=======================")
    ...
    selected_account = choose_tiktok_account(accounts)

    if selected_account is None:
        return

    posts_data = fetch_tiktok_posts(api_key, requests)
    ...
    print(f"Already published TikTok videos hidden: {already_published_count}")
    print(f"Unpublished TikTok videos available: {len(unpublished_videos)}")

    if not unpublished_videos:
        print("All exported videos have already been submitted to TikTok.")
        return
